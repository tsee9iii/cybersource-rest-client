...
class SandboxTester {
  ...
  async initialize() {
    console.log("🔧 Initializing CyberSource Sandbox Testing...\n");
    ...
    console.log(`   API Key ID: ${process.env.CYBERSOURCE_API_KEY_ID}`);
    console.log(
    ...
    this.module = await Test.createTestingModule({
      imports: [
        CyberSourceModule.forRoot({
          ...
          environment:
            (process.env.CYBERSOURCE_ENVIRONMENT as any) || "sandbox",
        }),
      ],
    }).compile();
    ...
    this.cyberSourceService =
      this.module.get<CyberSourceService>(CyberSourceService);
    this.customerService = this.module.get<CustomerService>(CustomerService);
    this.customerPaymentInstrumentService =
      this.module.get<CustomerPaymentInstrumentService>(
        CustomerPaymentInstrumentService
      );
    this.paymentInstrumentService = this.module.get<PaymentInstrumentService>(
      PaymentInstrumentService
    );
    this.instrumentIdentifierService =
      this.module.get<InstrumentIdentifierService>(InstrumentIdentifierService);
    this.tokenizedCardService =
      this.module.get<TokenizedCardService>(TokenizedCardService);
  ...
  async testBasicConnectivity() {
    ...
    // Test 2: List customers to verify read operations
        ...
        const instrument =
          await this.paymentInstrumentService.createPaymentInstrument({
            ...
            billTo: {
              ...
              postalCode: "12345",
              country: "US",
              email: `standalone-test-${Date.now()}@example.com`,
            },
          });
        testPaymentInstrumentId = instrument.id;
        return instrument;
      }
    );
  ...
  async testInstrumentIdentifierServices() {
    ...
    // Test 2: List payment instruments (BIN lookup)
